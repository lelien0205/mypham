...
class Master extends DBConnection
{
	...
	function save_brand()
	{
		...
		if ($save) {
			...
			$resp['status'] = 'success';
			if (empty($id))
				$resp['msg'] = "Đã lưu thành công Thương hiệu mới.";
			else
	...
	function save_category()
	{
		...
		if ($save) {
			$resp['status'] = 'success';
			if (empty($id))
				$this->settings->set_flashdata('success', "Danh mục mới đã được lưu.");
			else
				$this->settings->set_flashdata('success', "Danh mục đã được cập nhật.");
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error . "[{$sql}]";
		}
	...
	function delete_category()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Đã xóa danh mục.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_product()
	{
		...
		if ($save) {
			...
			if (!isset($resp)) {
				$resp['status'] = 'success';
				if (empty($id))
					$this->settings->set_flashdata('success', "Sản phẩm mới đã được lưu.");
				else
					$this->settings->set_flashdata('success', "Sản phẩm đã được cập nhật.");
			}
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error . "[{$sql}]";
	...
	function delete_product()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Đã xóa sản phẩm.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_inventory()
	{
		...
		if ($save) {
			$resp['status'] = 'success';
			if (empty($id))
				$this->settings->set_flashdata('success', "Kho mới đã được lưu.");
			else
				$this->settings->set_flashdata('success', "Kho đã được cập nhật.");
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error . "[{$sql}]";
		}
	...
	function delete_inventory()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Đã xóa kho.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function register()
	{
		...
		if ($save) {
			...
			if (empty($id))
				...
				$this->settings->set_flashdata('success', "Tài khoản đã được cập nhật.");
	...
	function delete_order()
	{
		...
		if ($delete) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Đã xóa đơn hàng");
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error . "[{$sql}]";
		}
		...
		if ($save_order) {
			...
			if ($save_olist) {
				...
				if ($this->capture_err())
					return $this->capture_err();
				$resp['status'] = 'success';
				$this->settings->set_flashdata('success', "Đặt hàng thành công.");
			} else {
				$resp['status'] = 'failed';
	...
	function update_order_status()
	{
		...
		if ($update) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata("success", "Đã cập nhật trạng thái đơn hàng.");
		} else {
			$resp['status'] = 'failed';
	...
	function pay_order()
	{
		...
		if ($update) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata("success", "Đã cập nhật trạng thái thanh toán.");
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error;
		}
	...
	function update_account()
	{
		...
		if ($save) {
			...
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', 'Thông tin tài khoản của bạn đã được cập.');
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function update_client()
	{
		...
		if ($save) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', 'Thông tin khách hàng đã được cập nhật.');
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_client()
	{
		...
		if ($delete) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Đã xóa khách hàng");
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error;
		}
    ...
    {
        extract($_POST);
        $insert = $this->conn->query("Insert into `contact` (`email`, `title`, `contact_content`) values('$email', '$title', '$contact_content')");
        if ($insert) {
            $resp['status'] = 'success';
            $this->settings->set_flashdata('success', "Đã gửi liên hệ thành công");
        } else {
            $resp['status'] = 'failed';
            $resp['err'] = $this->conn->error;
        }
